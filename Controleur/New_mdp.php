...
if ($resVerifPassword_Equality==0){
    header('Location: ../MDP/change_mdp.php?erreur=2'); // Erreur le mot de passe et sa confirmation sont différents
}

elseif ($resVerifPassword_Lenght==0){
    header('Location: ../MDP/change_mdp.php?erreur=1'); // Erreur le mot de passe à moins de 8 caractères
}

elseif($resVerifPassword_Lowercase==0){
    header('Location: ../MDP/change_mdp.php?erreur=3'); // Erreur pas de minuscule
}

elseif($resVerifPassword_Number==0){
    header('Location: ../MDP/change_mdp.php?erreur=4'); // Erreur pas de numéro
}

elseif($resVerifPassword_Uppercase==0){
    header('Location: ../MDP/change_mdp.php?erreur=5'); // Erreur pas de majuscule
}
